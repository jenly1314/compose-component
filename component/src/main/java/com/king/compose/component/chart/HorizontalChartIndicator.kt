 ...
 * @param cap 画笔结尾的样式
 * @param reverse 是否反向绘制；默认为false，表示从左往右的顺序绘制；当设置为true时，则表示从右往左的顺序绘制
 */
@Composable
fun HorizontalChartIndicator(
    values: List<Float>,
    colors: List<Color>,
            ...
            values.reversed().forEachIndexed { index, value ->
                ...
                if (value != 0f) {
                    drawLine(
                        ...
                        start = Offset(lastStartProgress, offset),
                        end = Offset(lastEndProgress, offset),
                        strokeWidth = thickness,
                        cap = cap
                    )
                }
